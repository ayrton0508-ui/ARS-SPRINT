...
test('version consistency',()=>{const c=fs.readFileSync(new URL('./config.js',import.meta.url),'utf8');const r=fs.readFileSync(new URL('./README.md',import.meta.url),'utf8');assert.match(c,/PRO 47/);assert.match(r,/PRO 47/)});

test('versioned service worker registration',()=>{const h=fs.readFileSync(new URL('./index.html',import.meta.url),'utf8');assert.match(h,/sw\.js\?v=47/)});
test('2D gate engine wired',()=>{const a=fs.readFileSync(new URL('./app.js',import.meta.url),'utf8');assert.ok(a.includes('crossedGate2D'));assert.ok(a.includes('interpolateCross2D'))});
