...
export function fitProjectiveMap(gates,direction=1){
  const pts=GATE_DISTANCES.map(m=>({m,x:Number(gates?.[m])}));
  ...
  // 1-D projective camera model: x = (a*m+b)/(c*m+1).
  // Solve the overdetermined 4-point system by normal equations.
  const A=pts.map(p=>[p.m,1,-p.m*p.x]), y=pts.map(p=>p.x);
  ...
  const predict=m=>(a*m+b)/(c*m+1);
  const err=Math.max(...pts.map(p=>Math.abs(predict(p.m)-p.x)));
  if(!Number.isFinite(err)||err>0.02)return null;
  return {a,b,c,error:err,direction};
}

export function mapMetersToPx(m,gates,direction=1){
  const model=fitProjectiveMap(gates,direction); if(!model||!Number.isFinite(m))return null;
  const den=model.c*m+1; if(Math.abs(den)<1e-8)return null;
  return (model.a*m+model.b)/den;
}

export function mapPxToMeters(x,gates,direction=1){
  if(!Number.isFinite(x)||!gates)return null;
  const model=fitProjectiveMap(gates,direction);
  if(model){const den=x*model.c-model.a;if(Math.abs(den)>1e-8){const m=(model.b-x)/den;if(Number.isFinite(m)&&m>=-0.25&&m<=20.25)return m;}}
