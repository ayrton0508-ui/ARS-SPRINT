...
function chooseLivePose(poses){const list=(poses||[]).map((lm,index)=>{const t=torsoFromLandmarks(lm);if(!t)return null;const xs=lm.map(p=>p?.x).filter(Number.isFinite),ys=lm.map(p=>p?.y).filter(Number.isFinite);const area=xs.length?(Math.max(...xs)-Math.min(...xs))*(Math.max(...ys)-Math.min(...ys)):0;return {lm,t,index,score:poseScore(lm),area}}).filter(Boolean);if(!list.length)return null;const lock=state.live?.lockPoint;if(lock){list.sort((a,b)=>{const da=(a.t.x-lock.x)**2+(a.t.y-lock.y)**2,db=(b.t.x-lock.x)**2+(b.t.y-lock.y)**2;return (da-db)*5+(b.score-a.score)*.5+(b.area-a.area)*.25;});return list[0]}list.sort((a,b)=>(b.score-b.score)+(b.area-a.area)+(b.score-a.score)*.35);return list[0];}
async function loadPoseAI(){
  ...
  poseAiLoading=(async()=>{try{
    ...
    const models=['https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_lite/float16/1/pose_landmarker_lite.task','https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_full/1/pose_landmarker_full.task'];let lastErr=null;
    for(const modelAssetPath of models){try{poseAiTask=await mod.PoseLandmarker.createFromOptions(vision,{baseOptions:{modelAssetPath,delegate:'GPU'},runningMode:'VIDEO',numPoses:3,minPoseDetectionConfidence:.15,minPosePresenceConfidence:.15,minTrackingConfidence:.15});break}catch(e){lastErr=e;try{poseAiTask=await mod.PoseLandmarker.createFromOptions(vision,{baseOptions:{modelAssetPath},runningMode:'VIDEO',numPoses:3,minPoseDetectionConfidence:.15,minPosePresenceConfidence:.15,minTrackingConfidence:.15});break}catch(e2){lastErr=e2}}}
...
function liveDraw(){
  ...
  const total=d, marks=[0,5,10,20].filter(m=>m<=Math.max(total,20));
  ...
  const x0=sx,x20=fx;
  const visibleMax=Math.max(20,total);
  const xAt=(m)=>x0+(x20-x0)*(m/visibleMax);
  // Ground plane / lane trapezoid, similar to a track projection overlay.
  ctx.save();
  ctx.fillStyle='rgba(255,208,0,.10)';ctx.beginPath();ctx.moveTo(x0,bottomY);ctx.lineTo(x20,bottomY);ctx.lineTo(x20-ctx.measureText('x').width+0,topY);ctx.lineTo(x0+(x20-x0)*.06,topY);ctx.closePath();ctx.fill();
  ...
  for(const L of [{x:sx,label:'🟢 INICIO · 0.00 m',stroke:'#20d68b'},{x:fx,label:`🏁 FINAL · ${total.toFixed(2)} m`,stroke:'#fff'}]){
    const bw=Math.min(210,w-8),bx=Math.max(4,Math.min(w-bw,L.x+7));ctx.fillStyle='rgba(10,10,10,.94)';ctx.fillRect(bx,cr.top+12,bw,31);ctx.fillStyle=L.stroke;ctx.font='900 13px sans-serif';ctx.fillText(L.label,bx+9,cr.top+33);
  }
  ...
  ctx.fillStyle='rgba(10,10,10,.9)';ctx.fillRect(Math.max(4,mid-58),cr.top+cr.height-92,116,24);ctx.fillStyle='#fff';ctx.font='900 12px sans-serif';ctx.textAlign='center';ctx.fillText(`${total.toFixed(2)} m`,mid,cr.top+cr.height-75);ctx.textAlign='start';
...
function bindLiveCamera(){if(!$('startCamera'))return;$('startCamera').onclick=startLiveCamera;$('armLive').onclick=armLiveMeasurement;$('stopCamera').onclick=()=>stopLiveCamera();$('liveVideo').addEventListener('loadedmetadata',liveDraw);window.addEventListener('resize',liveDraw);bindLiveOverlay();$('retryPoseLive')?.addEventListener('click',()=>{poseAiTask=null;poseAiLoading=null;loadPoseAI().catch(()=>{})});loadPoseAI().catch(()=>{})}
