...
state.live=state.live||{stream:null,armed:false,running:false,lastX:null,lastT:null,startTime:null,finishTime:null,observedFps:0,frames:0,body:null,raf:0,poseBusy:false,sourceFps:null,width:0,height:0,direction:null,armedAt:0,lastMediaTime:null,lastPerf:0,ring:[],ringMax:300,ringSlice:2,ringHeight:360,photoUrl:null};
function liveDistanceX(){return state.track.finishX??.88}
function liveSetStatus(text){$('liveInstruction')&&($('liveInstruction').textContent=text)}
function liveDraw(){if($('liveAthleteChip')){const a=athlete($('athleteSelect')?.value);$('liveAthleteChip').textContent=a?`Deportista: ${a.name} · ${distance()} m`:'Deportista: —'}if($('liveCalibrationText'))$('liveCalibrationText').textContent=`Protocolo físico: ${Number(distance()).toFixed(2)} m. Ajusta INICIO/FINAL sobre la imagen y confirma que las marcas físicas realmente están separadas ${Number(distance()).toFixed(2)} m.`;if($('liveVideo')&&!$('liveVideo').videoWidth)return;const v=$('liveVideo'),c=$('liveOverlay');if(!v||!c||!v.videoWidth)return;const r=v.getBoundingClientRect();const w=r.width,h=r.height,dpr=devicePixelRatio||1;c.width=Math.max(1,Math.round(w*dpr));c.height=Math.max(1,Math.round(h*dpr));const ctx=c.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);const sx=(state.track.startX??.12)*w,fx=liveDistanceX()*w;ctx.save();ctx.strokeStyle='#ffd000';ctx.lineWidth=2;ctx.setLineDash([8,6]);ctx.beginPath();ctx.moveTo(sx,0);ctx.lineTo(sx,h);ctx.moveTo(fx,0);ctx.lineTo(fx,h);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#111';ctx.font='700 12px sans-serif';ctx.fillText('INICIO',Math.max(4,sx+5),18);ctx.fillText(`FINAL ${distance()} m`,Math.max(4,fx-85),18);ctx.restore();const b=state.live.body;if(b){const x=b.x*w,y=b.y*h;ctx.beginPath();ctx.arc(x,y,10,0,Math.PI*2);ctx.strokeStyle='#ffd000';ctx.lineWidth=3;ctx.stroke();ctx.beginPath();ctx.moveTo(x-18,y);ctx.lineTo(x+18,y);ctx.moveTo(x,y-18);ctx.lineTo(x,y+18);ctx.stroke();ctx.fillStyle='#111';ctx.fillRect(Math.max(0,x+14),Math.max(0,y-13),135,25);ctx.fillStyle='#fff';ctx.font='700 12px sans-serif';ctx.fillText(`CUERPO · ${(b.visibility*100).toFixed(0)}%`,x+20,y+4)}}
function liveUpdateKpis(){if($('liveFps'))$('liveFps').textContent=state.live.sourceFps?state.live.sourceFps.toFixed(1)+' fps':'—';if($('liveObservedFps'))$('liveObservedFps').textContent=state.live.observedFps?state.live.observedFps.toFixed(1)+' fps':'—';if($('liveBodyState'))$('liveBodyState').textContent=state.live.body?(state.live.body.visibility>=.45?'Detectado':'Baja confianza'):'No detectado';if($('liveMeasureState'))$('liveMeasureState').textContent=state.live.finishTime!=null?'FINALIZADO':state.live.startTime!=null?'CORRIENDO':state.live.armed?'ARMADO':'Listo'}
...
async function startLiveCamera(){if(!navigator.mediaDevices?.getUserMedia)return alert('Este navegador no permite acceso a la cámara. Usa HTTPS y Safari/Chrome actualizado.');try{stopLiveCamera(false);if(!poseAiTask)await loadPoseAI();const facing=$('cameraFacing')?.value||'environment';const constraints={audio:false,video:{facingMode:{ideal:facing},width:{ideal:1920},height:{ideal:1080},frameRate:{ideal:120,max:240}}};let stream;try{stream=await navigator.mediaDevices.getUserMedia(constraints)}catch(e){stream=await navigator.mediaDevices.getUserMedia({audio:false,video:{facingMode:{ideal:($('cameraFacing')?.value||'environment')}}})}state.live.stream=stream;const v=$('liveVideo');v.srcObject=stream;await v.play();const track=stream.getVideoTracks()[0],settings=track.getSettings();state.live.cameraFacing=settings.facingMode||($('cameraFacing')?.value||'environment');state.live.sourceFps=Number(settings.frameRate)||null;state.fps=state.live.sourceFps;state.live.width=Number(settings.width)||v.videoWidth;state.live.height=Number(settings.height)||v.videoHeight;state.live.armed=false;state.live.running=true;state.live.startTime=null;state.live.finishTime=null;state.live.direction=null;state.live.armedAt=0;state.live.lastMediaTime=null;state.live.lastPerf=0;state.live.observedFps=0;state.live.ring=[];state.live.body=null;state.start=null;state.finish=null;state.track.torso=null;state.track.auto=false;$('liveStatus').textContent='CÁMARA ACTIVA';$('startCamera').disabled=true;$('armLive').disabled=false;$('stopCamera').disabled=false;if($('calibrateBtn'))$('calibrateBtn').disabled=false;$('fpsInput').value='custom';$('customFpsWrap').classList.remove('hidden');$('customFps').value=state.live.sourceFps||'';$('sourceFps').textContent=state.live.sourceFps?state.live.sourceFps.toFixed(1)+' fps (cámara)':'No disponible';liveSetStatus(`${state.live.cameraFacing==='user'?'🤳 Frontal':'🔙 Trasera'} activa · ${state.live.width||'?'}×${state.live.height||'?'} · ${state.live.sourceFps?state.live.sourceFps.toFixed(1)+' fps':'FPS no informado'}. El FPS mostrado proviene de la pista de cámara.`);liveUpdateKpis();liveSchedule();if($('athleteSelect')?.value)setTimeout(()=>{if(state.live.running&&!state.live.armed&&!state.live.finishTime)armLiveMeasurement()},300)}catch(e){console.error(e);alert('No se pudo abrir la cámara: '+(e.message||e.name))}}
function armLiveMeasurement(){if(!state.live.stream)return alert('Abre la cámara primero.');if(!$('athleteSelect').value)return alert('Selecciona un deportista antes de armar la medición.');state.live.armed=true;state.live.startTime=null;state.live.finishTime=null;state.live.lastX=null;state.live.lastT=null;state.live.lastMediaTime=null;state.live.observedFps=0;state.live.direction=null;state.live.armedAt=performance.now();state.live.ring=[];state.start=state.finish=null;state.gates={};$('armLive').textContent='🔴 Midiendo…';$('armLive').disabled=true;liveSetStatus(`MEDICIÓN ARMADA · ${distance()} m · esperando movimiento del cuerpo y cruce de INICIO.`);liveUpdateKpis()}
function liveCrossed(a,b,line,dir){return dir>0?(a<line&&b>=line):(a>line&&b<=line)}
async function liveProcessFrame(media,perf){if(!state.live.running)return;const prevMedia=state.live.lastMediaTime;if(prevMedia!=null){const dt=media-prevMedia;if(dt>0){const inst=1/dt;state.live.observedFps=state.live.observedFps?state.live.observedFps*.85+inst*.15:inst}}state.live.lastMediaTime=media;state.live.lastPerf=perf;state.live.frames++;if(state.live.armed)liveCaptureSlice();if(!state.live.poseBusy&&poseAiTask&&$('liveVideo').readyState>=2){state.live.poseBusy=true;try{const r=poseAiTask.detectForVideo($('liveVideo'),Math.round(media*1000));const torso=torsoFromLandmarks(r?.landmarks?.[0]);if(torso){const prev=state.live.lastX;state.live.body=torso;state.track.torso=torso;const startLine=state.track.startX??.12,finishLine=liveDistanceX();if(state.live.armed&&prev!=null&&!state.live.startTime){const dx=torso.x-prev;if(!state.live.direction&&Math.abs(dx)>=.006)state.live.direction=dx>0?1:-1;if(state.live.direction&&performance.now()>state.live.armedAt+500&&torso.visibility>=.45&&liveCrossed(prev,torso.x,startLine,state.live.direction)){const alpha=Math.max(0,Math.min(1,(startLine-prev)/(torso.x-prev||1)));const eventTime=prevMedia!=null?prevMedia+alpha*(media-prevMedia):media;state.live.startTime=eventTime;state.start={time:eventTime,frame:state.live.sourceFps?Math.round(eventTime*state.live.sourceFps):null,mode:'live-pose-interpolated',bodyDetected:true,visibility:torso.visibility};liveSetStatus(`🟢 INICIO detectado · ${media.toFixed(3)} s · dirección ${state.live.direction>0?'→':'←'}`)}}else if(state.live.armed&&state.live.startTime!=null&&!state.live.finishTime&&prev!=null&&state.live.direction&&torso.visibility>=.45&&liveCrossed(prev,torso.x,finishLine,state.live.direction)){const alpha=Math.max(0,Math.min(1,(finishLine-prev)/(torso.x-prev||1)));const eventTime=prevMedia!=null?prevMedia+alpha*(media-prevMedia):media;state.live.finishTime=eventTime;state.finish={time:eventTime,frame:state.live.sourceFps?Math.round(eventTime*state.live.sourceFps):null,mode:'live-pose-interpolated',bodyDetected:true,visibility:torso.visibility};state.live.armed=false;$('armLive').disabled=false;$('armLive').textContent='⏱️ Nueva medición';buildLivePhotoFinish();liveSetStatus(`🏁 FINAL detectado · ${state.finish.time.toFixed(3)} s · ${(state.finish.time-state.start.time).toFixed(3)} s · evidencia generada · revisar y guardar.`);updateMarks();updateResult();updateSaveState()}state.live.lastX=torso.x}else if(state.live.armed){state.live.lastX=null}}catch(e){console.warn('live pose',e)}finally{state.live.poseBusy=false}}liveUpdateKpis();liveDraw()}
...
function bindLiveCamera(){if(!$('startCamera'))return;$('startCamera').onclick=startLiveCamera;$('armLive').onclick=armLiveMeasurement;$('stopCamera').onclick=()=>stopLiveCamera();$('liveVideo').addEventListener('loadedmetadata',liveDraw);window.addEventListener('resize',liveDraw);bindLiveOverlay();loadPoseAI().catch(()=>{})}
