...
const state={db:{version:'10.0',athletes:[],history:[]},videoUrl:null,videoFile:null,fps:null,start:null,finish:null,lastFrameMeta:null,rvfcId:null,series:{active:false,total:0,current:0,attempts:[]},gates:{},vision:{selectedGate:5,positions:{},proposals:{},frameImage:null,accepted:false,validatedGates:{},manualGates:{}}};
...
function renderProfile(){const a=athlete($('profileSelect').value);if(!a){$('profileContent').innerHTML='<div class="status">Crea un deportista para generar su ficha.</div>';return}const hs=state.db.history.filter(h=>h.athleteId===a.id).sort((x,y)=>x.createdAt.localeCompare(y.createdAt));const stat=[5,10,20].map(d=>{const r=bestFor(a.id,d);return `<div class="stat-box"><span>${d} m</span><b>${r?r.time.toFixed(3)+' s':'—'}</b><small>${r?r.ms.toFixed(2)+' m/s':''}</small></div>`}).join('');$('profileContent').innerHTML=`<div class="profile-header"><div><h1>${esc(a.name)}</h1><p>${esc(a.sport||'')} · ${esc(a.category||'')} · ${esc(a.sex||'')} · ${esc(a.dob||'')}</p></div><div><b>ARS SPRINT 10.0</b><br><small>Informe generado ${new Date().toLocaleDateString('es-PE')}</small></div></div><div class="stat-grid">${stat}</div><h3>Historial de evaluaciones</h3><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Prueba</th><th>Intento</th><th>Tiempo</th><th>m/s</th><th>km/h</th><th>Calidad</th></tr></thead><tbody>${hs.map(h=>`<tr><td>${new Date(h.createdAt).toLocaleDateString('es-PE')}</td><td>${h.distance} m</td><td>${h.attempt}</td><td>${h.time.toFixed(3)} s</td><td>${h.ms.toFixed(2)}</td><td>${h.kmh.toFixed(2)}</td><td>${esc(h.quality)}</td></tr>`).join('')||'<tr><td colspan="7">Sin evaluaciones.</td></tr>'}</tbody></table></div><h3>Observaciones</h3><p>${esc(a.notes||'Sin observaciones.')}</p>`}
...
function bindGates(){document.querySelectorAll('.gateBtn').forEach(b=>b.onclick=()=>{const d=Number(b.dataset.gate),v=$('video');if(!v.duration)return alert('Carga un vídeo primero.');if(d>distance()&&distance()<=40)return alert('Esta puerta está fuera de la distancia seleccionada.');state.gates[String(d)]={time:activeFps()?Math.round(v.currentTime*activeFps())/activeFps():v.currentTime,frame:activeFps()?Math.round(v.currentTime*activeFps()):null};updateGates()});$('clearGates').onclick=()=>{state.gates={};updateGates()}}
...
function bindVideo(){const v=$('video');$('videoInput').onchange=e=>{const file=e.target.files?.[0];if(!file)return;if(state.videoUrl)URL.revokeObjectURL(state.videoUrl);state.videoFile=file;state.videoUrl=URL.createObjectURL(file);v.src=state.videoUrl;v.load();state.start=state.finish=null;state.gates={};state.lastFrameMeta=null;state.rvfcId=null;$('videoStatusPill').textContent='CARGANDO';updateMarks();updateResult()};v.onloadedmetadata=()=>{$('resolution').textContent=`${v.videoWidth}×${v.videoHeight}`;$('videoMeta').textContent=`${state.videoFile?.name||'Vídeo'} · ${(state.videoFile?.size/1048576||0).toFixed(1)} MB · ${v.duration.toFixed(3)} s`;$('videoStatusPill').textContent='LISTO';updateClock();startRVFC()};v.ontimeupdate=updateClock;v.onplay=()=>{$('playPause').textContent='⏸ Pausar'};v.onpause=()=>{$('playPause').textContent='▶ Reproducir'};$('playPause').onclick=()=>v.paused?v.play():v.pause();$('playbackRate').onchange=e=>v.playbackRate=Number(e.target.value);$('back1').onclick=()=>stepFrames(-1);$('back10').onclick=()=>stepFrames(-10);$('forward1').onclick=()=>stepFrames(1);$('forward10').onclick=()=>stepFrames(10)}
function startRVFC(){const v=$('video');if(!('requestVideoFrameCallback' in HTMLVideoElement.prototype))return;if(state.rvfcId!==null&&v.cancelVideoFrameCallback){v.cancelVideoFrameCallback(state.rvfcId);state.rvfcId=null}const cb=(now,meta)=>{state.lastFrameMeta=meta;state.rvfcId=v.requestVideoFrameCallback(cb)};state.rvfcId=v.requestVideoFrameCallback(cb)}
...
function bindSave(){ $('saveCurrent').onclick=()=>{if(state.series.active){alert('La serie está activa. Guarda el intento con «Siguiente intento» y finaliza la serie.');return}const m=currentMeasurement();if(!m)return;addMeasurement(m);$('validationNote').textContent='✓ Intento guardado en el historial.';$('saveCurrent').disabled=true;state.start=state.finish=null;state.gates={};state.vision.manualGates={};state.vision.validatedGates={};updateMarks();updateGates();updateResult();updateAudit()}}
...
function openTab(id){document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active',t.dataset.tab===id));document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===id));if(id==='profile')renderProfile();if(id==='dashboard')renderDashboard()}
...
document.addEventListener('DOMContentLoaded',()=>{load();bindTabs();bindVideo();bindFps();bindGates();bindSetup();bindSeries();bindSave();bindAthletes();bindHistory();bindBackup();bindOther();bindDashboard();bindVision();bindValidation();bindProtocol();renderAll();updateGates()});
