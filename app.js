...
function liveDistanceX(){return .88}
function liveSetStatus(text){$('liveInstruction')&&($('liveInstruction').textContent=text)}
function liveDraw(){const v=$('liveVideo'),c=$('liveOverlay');if(!v||!c||!v.videoWidth)return;const r=v.getBoundingClientRect();const w=r.width,h=r.height,dpr=devicePixelRatio||1;c.width=Math.max(1,Math.round(w*dpr));c.height=Math.max(1,Math.round(h*dpr));const ctx=c.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);const sx=(state.track.startX??.12)*w,fx=liveDistanceX()*w;ctx.save();ctx.strokeStyle='#ffd000';ctx.lineWidth=2;ctx.setLineDash([8,6]);ctx.beginPath();ctx.moveTo(sx,0);ctx.lineTo(sx,h);ctx.moveTo(fx,0);ctx.lineTo(fx,h);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#111';ctx.font='700 12px sans-serif';ctx.fillText('INICIO',Math.max(4,sx+5),18);ctx.fillText(`FINAL ${distance()} m`,Math.max(4,fx-85),18);ctx.restore();const b=state.live.body;if(b){const x=b.x*w,y=b.y*h;ctx.beginPath();ctx.arc(x,y,10,0,Math.PI*2);ctx.strokeStyle='#ffd000';ctx.lineWidth=3;ctx.stroke();ctx.beginPath();ctx.moveTo(x-18,y);ctx.lineTo(x+18,y);ctx.moveTo(x,y-18);ctx.lineTo(x,y+18);ctx.stroke();ctx.fillStyle='#111';ctx.fillRect(Math.max(0,x+14),Math.max(0,y-13),135,25);ctx.fillStyle='#fff';ctx.font='700 12px sans-serif';ctx.fillText(`CUERPO · ${(b.visibility*100).toFixed(0)}%`,x+20,y+4)}}
