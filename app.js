...
const state={db:{version:'10.0',athletes:[],history:[]},videoUrl:null,videoFile:null,fps:null,start:null,finish:null,lastFrameMeta:null,rvfcId:null,series:{active:false,total:0,current:0,attempts:[]},gates:{},track:{startX:.12,finishX:.88,torso:null,direction:null,drag:null,auto:false,lastDetect:0},photoFinish:{url:null},vision:{selectedGate:5,positions:{},proposals:{},frameImage:null,accepted:false,validatedGates:{},manualGates:{}}};
...
function bindSave(){ $('saveCurrent').onclick=()=>{if(state.series.active){alert('La serie está activa. Guarda el intento con «Siguiente intento» y finaliza la serie.');return}if(!$('athleteSelect')?.value){alert('Selecciona un deportista. El historial no guardará resultados sin nombre.');return}const m=currentMeasurement();if(!m)return;$('validationNote').textContent='✓ Intento guardado en el historial.';$('saveCurrent').disabled=true;state.start=state.finish=null;state.gates={};state.vision.manualGates={};state.vision.validatedGates={};updateMarks();updateGates();updateResult();updateAudit()}}
...
function updateBodyStatus(){const el=$('bodyStatus');if(!el)return;el.textContent=state.track.torso?`🟢 Cuerpo detectado · visibilidad ${(state.track.torso.visibility*100).toFixed(0)}% · Inicio ${(state.track.startX*100).toFixed(0)}% · Final ${(getTrackFinishX()*100).toFixed(0)}%`:`⚪ Cuerpo: no detectado · coloca al atleta dentro del encuadre`}
...
document.addEventListener('DOMContentLoaded',()=>{load();bindTabs();bindVideo();bindFps();bindGates();bindSetup();bindSeries();bindSave();bindAthletes();bindHistory();bindBackup();bindOther();bindDashboard();bindVision();bindValidation();bindProtocol();renderAll();updateGates();openTab('evaluation');drawTrackingOverlay();updateBodyStatus();bindLiveCamera();});
...
async function loadPoseAI(){
  const status=$('poseAiStatus');
  if(poseAiTask)return poseAiTask;
  if(poseAiLoading)return poseAiLoading;
  status.textContent='Cargando modelo de pose…';
  poseAiLoading=(async()=>{
    try{
      ...
      poseAiTask=await mod.PoseLandmarker.createFromOptions(vision,{
        baseOptions:{
      ...
      status.textContent='✓ IA de pose cargada.';
      return poseAiTask;
    }catch(err){
      poseAiLoading=null; status.textContent='No se pudo cargar la IA. Revisa conexión a Internet.'; console.error(err); throw err;
    }
  })();
  return poseAiLoading;
}
function torsoFromLandmarks(lm){
  if(!lm||lm.length<25)return null;
  const ids=[11,12,23,24]; // shoulders + hips
  const pts=ids.map(i=>lm[i]).filter(p=>p&&Number.isFinite(p.x)&&Number.isFinite(p.y));
...
/* ARS SPRINT — LIVE UNIFIED CAMERA ENGINE */
state.live=state.live||{stream:null,armed:false,running:false,lastX:null,lastT:null,startTime:null,finishTime:null,observedFps:0,frames:0,body:null,raf:0,poseBusy:false};
function liveDistanceX(){const d=distance();if(d===5)return .88;if(d===10)return .88;if(d===20)return .88;return .88}
function liveSetStatus(text){$('liveInstruction')&&($('liveInstruction').textContent=text)}
function liveDraw(){const v=$('liveVideo'),c=$('liveOverlay');if(!v||!c||!v.videoWidth)return;const r=v.getBoundingClientRect();const w=r.width,h=r.height,dpr=devicePixelRatio||1;c.width=Math.max(1,Math.round(w*dpr));c.height=Math.max(1,Math.round(h*dpr));const ctx=c.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);const b=state.live.body;if(b){const x=b.x*w,y=b.y*h;ctx.beginPath();ctx.arc(x,y,10,0,Math.PI*2);ctx.strokeStyle='#ffd000';ctx.lineWidth=3;ctx.stroke();ctx.beginPath();ctx.moveTo(x-18,y);ctx.lineTo(x+18,y);ctx.moveTo(x,y-18);ctx.lineTo(x,y+18);ctx.stroke();ctx.fillStyle='#111';ctx.fillRect(Math.max(0,x+14),Math.max(0,y-13),135,25);ctx.fillStyle='#fff';ctx.font='700 12px sans-serif';ctx.fillText(`CUERPO · ${(b.visibility*100).toFixed(0)}%`,x+20,y+4)} }
function liveUpdateKpis(){if($('liveFps'))$('liveFps').textContent=state.live.sourceFps?state.live.sourceFps.toFixed(1)+' fps':'—';if($('liveObservedFps'))$('liveObservedFps').textContent=state.live.observedFps?state.live.observedFps.toFixed(1)+' fps':'—';if($('liveBodyState'))$('liveBodyState').textContent=state.live.body?'Detectado':'No detectado';if($('liveMeasureState'))$('liveMeasureState').textContent=state.live.finishTime!=null?'FINALIZADO':state.live.startTime!=null?'CORRIENDO':state.live.armed?'ARMADO':'Listo'}
async function startLiveCamera(){if(!navigator.mediaDevices?.getUserMedia)return alert('Este navegador no permite acceso a la cámara. Usa HTTPS y Safari/Chrome actualizado.');try{stopLiveCamera(false);if(!poseAiTask)await loadPoseAI();const constraints={audio:false,video:{facingMode:{ideal:'environment'},width:{ideal:1920},height:{ideal:1080},frameRate:{ideal:120,max:240}}};let stream;try{stream=await navigator.mediaDevices.getUserMedia(constraints)}catch(e){stream=await navigator.mediaDevices.getUserMedia({audio:false,video:{facingMode:'environment'}})}state.live.stream=stream;const v=$('liveVideo');v.srcObject=stream;await v.play();const track=stream.getVideoTracks()[0],settings=track.getSettings();state.live.sourceFps=Number(settings.frameRate)||null;state.fps=state.live.sourceFps;state.live.armed=false;state.live.running=true;state.live.startTime=null;state.live.finishTime=null;state.live.direction=null;state.live.autoReady=false;state.live.armedAt=0;state.live.cooldownUntil=0;state.start=null;state.finish=null;state.track.torso=null;state.track.auto=false;$('liveStatus').textContent='CÁMARA ACTIVA';$('startCamera').disabled=true;$('armLive').disabled=false;$('stopCamera').disabled=false;$('fpsInput').value='custom';$('customFpsWrap').classList.remove('hidden');$('customFps').value=state.live.sourceFps||'';$('sourceFps').textContent=state.live.sourceFps?state.live.sourceFps.toFixed(1)+' fps (cámara)':'No disponible';liveSetStatus(`Cámara activa · ${settings.width||'?'}×${settings.height||'?'} · ${state.live.sourceFps?state.live.sourceFps.toFixed(1)+' fps':'FPS no informado'}. Selecciona deportista y pulsa ARMAR: después detectaremos dirección, inicio y final automáticamente.`);liveUpdateKpis();liveLoop();if($('athleteSelect')?.value){setTimeout(()=>{if(state.live.running&&!state.live.armed&&!state.live.finishTime)armLiveMeasurement()},300)} }catch(e){console.error(e);alert('No se pudo abrir la cámara: '+(e.message||e.name))}}
function armLiveMeasurement(){if(!state.live.stream)return alert('Abre la cámara primero.');if(!$('athleteSelect').value)return alert('Selecciona un deportista antes de armar la medición.');state.live.armed=true;state.live.startTime=null;state.live.finishTime=null;state.live.lastX=null;state.live.lastT=null;state.live.frames=0;state.live.observedFps=0;state.live.direction=null;state.live.autoReady=true;state.live.armedAt=performance.now();state.start=state.finish=null;state.gates={};$('armLive').textContent='🔴 Midiendo…';$('armLive').disabled=true;liveSetStatus(`MEDICIÓN AUTOMÁTICA ARMADA · ${distance()} m · detectando dirección, INICIO y FINAL.`);liveUpdateKpis()}
function liveCrossed(a,b,line,dir){return dir>0?(a<line&&b>=line):(a>line&&b<=line)}
async function liveLoop(){const v=$('liveVideo');if(!state.live.running)return;const media=Number.isFinite(v.currentTime)?v.currentTime:performance.now()/1000;if(state.live.lastT!=null){const dt=media-state.live.lastT;if(dt>0){const inst=1/dt;state.live.observedFps=state.live.observedFps?state.live.observedFps*.85+inst*.15:inst}}state.live.lastT=media;state.live.frames++;if(!state.live.poseBusy&&poseAiTask&&v.readyState>=2){state.live.poseBusy=true;try{const r=poseAiTask.detectForVideo(v,Math.round(media*1000));const torso=torsoFromLandmarks(r?.landmarks?.[0]);if(torso){const prev=state.live.lastX;state.live.body=torso;state.track.torso=torso;const startLine=state.track.startX??.12,finishLine=liveDistanceX();if(state.live.armed&&prev!=null&&!state.live.startTime){const dx=torso.x-prev;if(!state.live.direction&&Math.abs(dx)>=.008)state.live.direction=dx>0?1:-1;if(state.live.direction&&performance.now()>state.live.armedAt+500&&liveCrossed(prev,torso.x,startLine,state.live.direction)){const alpha=Math.max(0,Math.min(1,(startLine-prev)/(torso.x-prev||1)));const tt=(media-(state.live.lastT?0:0));state.live.startTime=media;state.start={time:media,frame:state.live.sourceFps?Math.round(media*state.live.sourceFps):null,mode:'live-pose',bodyDetected:true,visibility:torso.visibility};liveSetStatus(`🟢 INICIO detectado · ${media.toFixed(3)} s · dirección ${state.live.direction>0?'→':'←'}`)}}else if(state.live.armed&&state.live.startTime!=null&&!state.live.finishTime&&prev!=null&&state.live.direction&&liveCrossed(prev,torso.x,finishLine,state.live.direction)){state.live.finishTime=media;state.finish={time:media,frame:state.live.sourceFps?Math.round(media*state.live.sourceFps):null,mode:'live-pose',bodyDetected:true,visibility:torso.visibility};state.live.armed=false;$('armLive').disabled=false;$('armLive').textContent='⏱️ Nueva medición';liveSetStatus(`🏁 FINAL detectado · ${media.toFixed(3)} s · ${((state.finish.time-state.start.time)).toFixed(3)} s · revisa y guarda.`);updateMarks();updateResult();updateSaveState()}state.live.lastX=torso.x}else if(state.live.armed){state.live.lastX=null}}catch(e){console.warn('live pose',e)}finally{state.live.poseBusy=false}}liveUpdateKpis();liveDraw();state.live.raf=requestAnimationFrame(liveLoop)}
function stopLiveCamera(reset=true){if(state.live.raf)cancelAnimationFrame(state.live.raf);state.live.raf=0;state.live.running=false;state.live.armed=false;state.live.poseBusy=false;if(state.live.stream){state.live.stream.getTracks().forEach(t=>t.stop());state.live.stream=null}const v=$('liveVideo');if(v)v.srcObject=null;if($('liveStatus'))$('liveStatus').textContent='CÁMARA APAGADA';if($('startCamera'))$('startCamera').disabled=false;if($('armLive')){$('armLive').disabled=true;$('armLive').textContent='⏱️ Armar medición'}if($('stopCamera'))$('stopCamera').disabled=true;if(reset)liveSetStatus('Cámara detenida.');liveUpdateKpis()}
function bindLiveCamera(){if(!$('startCamera'))return;$('startCamera').onclick=startLiveCamera;$('armLive').onclick=armLiveMeasurement;$('stopCamera').onclick=()=>stopLiveCamera();$('liveVideo').addEventListener('loadedmetadata',liveDraw);window.addEventListener('resize',liveDraw);loadPoseAI().catch(()=>{})}
