...
const state={db:{version:'V26.87',athletes:[],history:[]},videoUrl:null,videoFile:null,fps:null,start:null,finish:null,lastFrameMeta:null,rvfcId:null,series:{active:false,total:0,current:0,attempts:[]},gates:{},track:{startX:.12,finishX:.88,torso:null,direction:null,drag:null,auto:false,lastDetect:0},calibration:{target:5,estimated:null,quality:'No calibrado',saved:false,referenceWidthM:0.21},photoFinish:{url:null},vision:{selectedGate:5,positions:{},proposals:{},frameImage:null,accepted:false,validatedGates:{},manualGates:{}}};
...
if(x){const athletes=Array.isArray(x.athletes)?x.athletes.filter(a=>a&&a.id):[];const history=Array.isArray(x.history)?x.history.filter(h=>h&&h.athleteId):[];state.db={version:'V26.87',athletes,history,gates:x.gates&&typeof x.gates==='object'?x.gates:{}};save();}
...
function bindSetup(){
  // V26.87: the old checkbox checklist was removed from the main flow.
...
function printAthleteReport(){
  ...
  w.document.write(`<html><head><title>ARS SPRINT — Ficha</title><style>body{font-family:Arial;padding:32px}h1{margin-bottom:4px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.box{border:1px solid #ddd;padding:16px;border-radius:10px}.box b{font-size:22px;display:block;margin-top:6px}</style></head><body><h1>ARS SPRINT V26.87</h1><p><b>Deportista:</b> ${athleteName(a)}<br><b>Prueba:</b> ${d} m<br><b>Registros:</b> ${times.length}</p><div class="grid"><div class="box">Mejor<b>${best.toFixed(3)} s</b></div><div class="box">Promedio<b>${mean.toFixed(3)} s</b></div><div class="box">Último<b>${times[times.length-1].toFixed(3)} s</b></div></div><h2>Historial</h2><ol>${rows.map(h=>`<li>${historyDate(h)||'Sin fecha'} — ${formatSec(historyTimeFor(h,d))}</li>`).join('')}</ol><script>window.onload=()=>window.print()<\/script></body></html>`);
  w.document.close();
}
...
async function loadPoseAI(){
  ...
  poseAiLoading=(async()=>{try{
    ...
    const models=['https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_lite/float16/1/pose_landmarker_lite.task','https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_full/float16/1/pose_landmarker_full.task'];
    const options={runningMode:'VIDEO',numPoses:1,minPoseDetectionConfidence:.25,minPosePresenceConfidence:.25,minTrackingConfidence:.25};let lastErr=null;
    ...
    if(!poseAiTask){const detail=lastErr?.message||String(lastErr||'error desconocido');throw new Error(`No se pudo crear PoseLandmarker. Modelo/IA: ${detail}`)}
...
/* ARS SPRINT V26.87 — LIVE CAMERA / POSE ENGINE */
    ...
    try{
      ...
      if(torso&&raw){
        ...
        if(state.live.armed&&prev!=null&&state.live.startTime==null){
          ...
          // V26.87: Smart Sprint is the sole automatic event authority.
...
// V26.87: live engine retained; measurement controls remain explicit and safe.

/* ARS SPRINT V26.87 — SMART SPRINT SPECIFICATION LAYER
...
(function(){
  ...
  // V26.87: map image position to distance using the independently calibrated
  ...
  liveProcessFrame=async function(media,perf){
    ...
    // V26.87: automatic measurement readiness. Detection is automatic; START is still
  ...
  function smartCalibrate(){
    ...
    state.vision.calibration={version:'V26.87',positions:{0:startX,5:p5,10:p10,20:p20},ordered,cameraReady,score,resolution:`${w}×${h}`,fps,physicalDistanceConfirmed,createdAt:new Date().toISOString()};
