...
const state={db:{version:'V26.80',athletes:[],history:[]},videoUrl:null,videoFile:null,fps:null,start:null,finish:null,lastFrameMeta:null,rvfcId:null,series:{active:false,total:0,current:0,attempts:[]},gates:{},track:{startX:.12,finishX:.88,torso:null,direction:null,drag:null,auto:false,lastDetect:0},calibration:{target:5,estimated:null,quality:'No calibrado',saved:false,referenceWidthM:0.21},photoFinish:{url:null},vision:{selectedGate:5,positions:{},proposals:{},frameImage:null,accepted:false,validatedGates:{},manualGates:{}}};
...
if(x){state.db={version:'V26.80',athletes:Array.isArray(x.athletes)?x.athletes:[],history:Array.isArray(x.history)?x.history:[],gates:x.gates&&typeof x.gates==='object'?x.gates:{}};save()}
...
function updateSetup(){const ids=['setupLateral','setupLevel','setupFullTrack','setupStable','setupFpsKnown','setupPhysicalDistances'];const n=ids.filter(id=>$(id).checked).length;$('setupScore').textContent=n===6?'✓ Preparación completa':`${n}/6 controles completados`;}
...
function printAthleteReport(){
  ...
  w.document.write(`<html><head><title>ARS SPRINT — Ficha</title><style>body{font-family:Arial;padding:32px}h1{margin-bottom:4px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.box{border:1px solid #ddd;padding:16px;border-radius:10px}.box b{font-size:22px;display:block;margin-top:6px}</style></head><body><h1>ARS SPRINT V26.80</h1><p><b>Deportista:</b> ${athleteName(a)}<br><b>Prueba:</b> ${d} m<br><b>Registros:</b> ${times.length}</p><div class="grid"><div class="box">Mejor<b>${best.toFixed(3)} s</b></div><div class="box">Promedio<b>${mean.toFixed(3)} s</b></div><div class="box">Último<b>${times[times.length-1].toFixed(3)} s</b></div></div><h2>Historial</h2><ol>${rows.map(h=>`<li>${historyDate(h)||'Sin fecha'} — ${formatSec(historyTimeFor(h,d))}</li>`).join('')}</ol><script>window.onload=()=>window.print()<\/script></body></html>`);
  w.document.close();
}
...
/* ARS SPRINT V26.80 — LIVE CAMERA / POSE ENGINE */
...
function liveDraw(){
  ...
  if($('liveGuideFinish'))$('liveGuideFinish').textContent=`${d.toFixed(2)} m`;
  if($('liveCalibrationText'))$('liveCalibrationText').textContent=`Objetivo ${d.toFixed(2)} m. Coloca físicamente las marcas de INICIO y FINAL a esa distancia.`;
  const sx=Math.max(3,Math.min(97,(state.track.startX??.12)*100)),fx=Math.max(sx+5,Math.min(97,liveDistanceX()*100));
  ...
  [5,10].forEach(g=>{const el=$('liveGate'+g),gx=gateXForDistance(g);if(!el)return;const visible=distance()>=g&&gx!=null;el.style.left=visible?(gx*100)+'%':'-9999px';el.style.display=visible?'block':'none';});
  liveUpdateKpis();
}
...
async function startLiveCamera(){
  ...
  try{
    ...
    const constraints=[
      ...
      {audio:false,video:{facingMode:facing,width:{ideal:960},height:{ideal:540},frameRate:{ideal:30,max:30}}},
      {audio:false,video:{facingMode:{ideal:facing},frameRate:{max:30},width:{ideal:640},height:{ideal:480}}}
    ];
    ...
    try{
      ...
      if(torso&&raw){
        ...
        if(state.live.armed&&prev!=null&&state.live.startTime==null){
          ...
          // V26.80: Smart Sprint is the sole automatic event authority.
...
function bindLiveCamera(){
  ...
  $('startCamera').onclick=()=>{if(state.live?.finishTime){stopLiveCamera(false);startLiveCamera()}else if(state.live?.stream){armLiveMeasurement()}else startLiveCamera()};
  ...
  $('stopCamera').onclick=()=>stopLiveCamera();$('liveVideo').addEventListener('loadedmetadata',liveDraw);window.addEventListener('resize',liveDraw);$('retryPoseLiveBtn')?.addEventListener('click',()=>{poseAiTask=null;poseAiLoading=null;if($('liveAiHint'))$('liveAiHint').textContent='🧠 IA corporal: cargando…';loadPoseAI().catch(()=>{})});loadPoseAI().catch(()=>{});
}


// V26.80: live engine retained; measurement controls remain explicit and safe.

/* ARS SPRINT V26.80 — SMART SPRINT SPECIFICATION LAYER
...
(function(){
  ...
  // V26.80: map image position to distance using the independently calibrated
  ...
  liveProcessFrame=async function(media,perf){
    ...
    // V26.80: automatic measurement readiness. Detection is automatic; START is still
    ...
    if(state.live?.stream && !state.live.armed && state.live.startTime==null && state.live.finishTime==null &&
       state.live.stableHits>=12 && state.live.body?.visibility>=.45 && state.calibration?.saved){
      resetSmart();
  ...
  function smartCalibrate(){
    ...
    const d=20, v=$('liveVideo'), w=v?.videoWidth||0, h=v?.videoHeight||0, fps=state.live.sourceFps||0;
    const checks=['setupLateral','setupLevel','setupFullTrack','setupPerspective','setupStable','setupFpsKnown'];
    const checklist=checks.filter(id=>$(id)?.checked).length; const physicalDistanceConfirmed=!!$('setupPhysicalDistances')?.checked;
    const positions=state.vision?.positions||{};
    const p5=Number(positions['5']), p10=Number(positions['10']), p20=Number(positions['20']);
    const ordered=[p5,p10,p20].every(Number.isFinite) && p5>=0 && p10>p5+.02 && p20>p10+.02 && p20<=1;
    const startX=Number(state.track.startX??.12), finishX=Number(positions['20'] ?? state.track.finishX??.88);
    const span=Math.abs(finishX-startX);
    const cameraReady=w>=720 && h>=480 && fps>=24;
    const score=Math.min(100,(cameraReady?30:10)+(ordered?35:0)+(checklist*(30/6))+(state.live.stableHits>=6?10:0)+(span>=.25?10:0));
    state.calibration.target=d;
    state.calibration.estimated=null;
    state.calibration.quality=ordered&&checklist===checks.length&&cameraReady&&physicalDistanceConfirmed?'🟢 CALIBRACIÓN CORRECTA':score>=60?'🟡 CALIBRACIÓN A REVISAR':'🔴 CALIBRACIÓN INSUFICIENTE';
    state.calibration.saved=ordered&&checklist===checks.length&&cameraReady&&physicalDistanceConfirmed;
    state.track.finishX=Math.min(.97,Math.max(startX+.05,finishX));
    state.vision.calibration={version:'V26.80',positions:{0:startX,5:p5,10:p10,20:p20},ordered,cameraReady,checklist,score,resolution:`${w}×${h}`,fps,physicalDistanceConfirmed,createdAt:new Date().toISOString()};
    state.db.trackCalibration=state.db.trackCalibration||{};
    state.db.trackCalibration.smart={distance:20,positions:state.vision.calibration.positions,camera:state.live.cameraFacing||null,fps,resolution:`${w}×${h}`,score,validated:state.calibration.saved,physicalDistanceConfirmed,createdAt:new Date().toISOString()};
    save();
    if($('smartCalState'))$('smartCalState').textContent=state.calibration.saved?'🟢 CALIBRACIÓN CORRECTA':score>=60?'🟡 REVISAR CAPTURA':'🔴 CORREGIR CAPTURA';
    if($('smartCalDetail'))$('smartCalDetail').textContent=`0–5–10–20 m · ${w||'—'}×${h||'—'} · ${fps||'—'} fps · ${score}/100`;
    if($('calibrationState'))$('calibrationState').textContent=state.calibration.quality;
    if($('calQuality'))$('calQuality').textContent=ordered?`Puertas verificadas · ${score}/100`:`Faltan posiciones físicas 5/10/20 m · ${score}/100`;
    liveDraw();liveSetStatus(state.calibration.saved?'📐 CALIBRACIÓN VALIDADA · sistema listo para detección automática.':physicalDistanceConfirmed?'📐 Calibración incompleta · revisa captura y checklist.':'📐 Confirma físicamente las distancias 5–10–20 m antes de iniciar.');updateSmartUI();
  }
